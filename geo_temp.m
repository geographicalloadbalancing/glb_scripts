...
for i = 1:1:1
    ...
    for j = 1:1:1
        ...
        csvwrite('results/portfolio-rhc-x.csv',[x_rhc],0,0);
        %}
        ...
        csvwrite('results/portfolio-afhc-x.csv',[x_afhc],0,0);        
...
%             
    ...
    for so = 1:2:11
        ...
        %while done == 0
            ...
            T = S.*(0.1*(so-1)*capacity'*ones(1,144*7)) + W.*(wi*capacity'*ones(1,144*7));
...
figure;
plot(0:0.5:1,brown_opt(1:5:11),'k',0:0.5:1,brown_rhc(1:5:11),'r',0:0.5:1,brown_afhc(1:5:11),'b',0:0.5:1,brown_loc(1:5:11),'g')
xlabel('% of wind');
ylabel('brown energy consumption');
%ylim([0,1]);
legend('OPTIMAL','RHC', 'AFHC', 'LOCAL');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.8]);
print ('-depsc', 'figs/brownWind.eps');

figure;
plot(0:0.5:1,cost_opt(1:5:11),'k',0:0.5:1,cost_rhc(1:5:11),'r',0:0.5:1,cost_afhc(1:5:11),'b',0:0.5:1,cost_loc(1:5:11),'g')
xlabel('% of wind');
ylabel('total cost');
%ylim([0,1]);
legend('OPTIMAL','RHC', 'AFHC', 'LOCAL');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.8]);
print ('-depsc', 'figs/costWind.eps');

figure;
plot(0:0.5:1,delay_opt(1:5:11),'k',0:0.5:1,delay_rhc(1:5:11),'r',0:0.5:1,delay_afhc(1:5:11),'b',0:0.5:1,delay_loc(1:5:11),'g')
xlabel('% of wind');
ylabel('average delay');
%ylim([0,1]);
legend('OPTIMAL','RHC', 'AFHC', 'LOCAL');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.8]);
print ('-depsc', 'figs/delayWind.eps');
