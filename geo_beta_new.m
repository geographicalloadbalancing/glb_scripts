...
for i = 1:1:1
    ...
    for j = 1:1:10
        ...
        DCL = DCL;
        %M = 2 * (1+1./sqrt(j*[10.41 3.73 5.87 7.48 5.86 6.67 6.44 8.6 6.03 5.49]')).*floor(max(DCL'))';
        M = 2 *floor(max(DCL'))';
        capacity = mean(DCL');    
        ...
        lambda_t = load2';
        mu = ones(a,1);
        energy_cost = j*[10.41 3.73 5.87 7.48 5.86 6.67 6.44 8.6 6.03 5.49]';
        delay_cost = ones(a,1);
        beta = beta0*ones(a,1);
        prop_delay = delay;
        caps = M;
...
xlabel('energy price (1/\gamma)');
ylabel('total cost');
xlim([1,10]);
ylim([0,6e9]);
legend('GLB','RHC','AFHC','LOCAL','Location','NorthWest');
