    ...
    for j = 1:1:21
        ...
        if loc == 1
            for k = 1:1:a
                ...
                brown_k(k,j,i) = sum(max(0,x_loc(:,k,j,i)-Re(k,1:length)'));
            end
            cost_k(:,j,i) = cost_local;
...
print ('-depsc', 'figs/brownWind1.eps');
%}

figure;
plot(0:0.05:1,brown_k(2,1:1:21)/max(brown_k(2,1:1:21)),'k-',0:0.05:1,brown_k(6,1:1:21)/max(brown_k(6,1:1:21)),'k--',0:0.05:1,brown_k(3,1:1:21)/max(brown_k(3,1:1:21)),'k-.')
xlabel('wind ratio');
ylabel('relative cost');
%ylim([0,1]);
legend('Washington','Virginia','Oregon');
