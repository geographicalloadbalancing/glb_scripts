...
for i = 1:1:1
    ...
    for j = 1:1:21
        ...
        if loc == 1
            for k = 1:1:a
                [x_loc(:,k,j,i) cost_local(k,j) delay_local(k)] = hetero_opt(x0, DCL2(k,1:length)', mu(k), energy_cost(k), delay_cost(k), beta(k), prop_delay_loc(1,k), caps(k), Re(k,1:length)');
            end
    ...
    %csvwrite('results/PMR-compare1.csv',[cost_opt, cost_local', cost_loc],0,0);
end


figure;
plot(0:0.05:1,cost_local(2,1:1:21)/max(cost_local(2,1:1:21)),'k-',0:0.05:1,cost_local(6,1:1:21)/max(cost_local(6,1:1:21)),'k--',0:0.05:1,cost_local(3,1:1:21)/max(cost_local(3,1:1:21)),'k-.')
xlabel('wind ratio');
ylabel('relative cost');
ylim([0.79,1]);
set(gca,'XTick',[0:0.2:1]);
legend('Washington','Virginia','Oregon','Location','Northeast');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 2.8 2.1]);
print ('-depsc', 'figs/portfolio1.eps');
