...
for i = 1:1:9
    ...
    for j = 1:1:1
        ...
        so = i - wi;
        %re(j) = 0.5*(j-1);
        Re = (S.*(so*capacity'*ones(1,144*7)) + W.*(wi*capacity'*ones(1,144*7)));
        x0 = 0;
        lambda_t = load2';
        mu = ones(a,1);
...
print ('-depsc', 'figs/brownCapacity1.eps');
...
figure;
plot([0:0.5:2.5,3:1:10],[brown_opt(1),1e7*8.9763,brown_opt(2),1e7*0.6217, brown_opt(3),1e7*0.2488,brown_opt(4:1:11)]/brown_opt(1),'k-',...
    [0:0.5:2.5,3:1:10],[brown_rhc(1),1e7*8.8860,brown_rhc(2),1e7*0.6323, brown_rhc(3),1e7*0.2644,brown_rhc(4:1:11)]/brown_rhc(1),'r--',...
    [0:0.5:2.5,3:1:10],[brown_afhc(1),1e7*8.7773,brown_afhc(2),1e7*0.6022, brown_afhc(3),1e7*0.2538,brown_afhc(4:1:11)]/brown_afhc(1),'m-.',...
    [0:0.5:2.5,3:1:10],[brown_loc(1),1e7*8.9225,brown_loc(2),1e7*1.7786, brown_loc(3),1e7*0.9530,brown_loc(4:1:11)]/brown_loc(1),'b--',...
    0:1:10,0.1*ones(11,1),'k:',0:1:10,0.05*ones(11,1),'k:')
xlabel('capacity');
...
ylim([0,0.5]);
legend('GLB','RHC','AFHC','LOCAL');
set(gca,'XTick',[0:0.5:5], 'FontSize', 8);
set(gca,'YTick',[0:0.05:0.5], 'FontSize', 8);
...
legend('GLB','RHC','AFHC','LOCAL');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.8]);
print ('-depsc', 'figs/costCapacity1.eps');
...
legend('GLB','RHC','AFHC','LOCAL');
set (gcf, 'PaperUnits', 'inches', 'PaperPosition', [0.1 0 3.6 2.8]);
print ('-depsc', 'figs/delayCapacity1.eps');